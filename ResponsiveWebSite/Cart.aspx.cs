    ...
    public partial class Cart : System.Web.UI.Page
    {
        ...
        private void BindCartProducts()
        {
            if (Request.Cookies["CartPID"] != null)
            {
                ...
                {
                    h5NoItems.InnerText = "MY CART (" + CookieDataArray.Length + " Item)";
                        ...
                        String CS = ConfigurationManager.ConnectionStrings["MyDatabaseConnectionString1"].ConnectionString;
                            using (SqlConnection con = new SqlConnection(CS))
                            {
                                using (SqlCommand cmd = new SqlCommand("select A.*,dbo.getSizeName(" + SizeID + ") as SizeNamee,"
                                    + SizeID + " as SizeIDD,SizeData.Name,SizeData.Extention from tblProducts A cross apply(select top 1 B.Name, B.Extention from tblProductImages B where B.PID = A.PID) SizeData where A.PID="
                                    + PID + "", con))
                                {
                ...
                {
                    h5NoItems.InnerText = "Yout Shopping Cart is empty ";
