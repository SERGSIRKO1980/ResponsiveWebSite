...
namespace ResponsiveWebSite
{
    public partial class AddSize : System.Web.UI.Page
    {
        ...
        private void BindBrandsRptr()
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("select A.*, B.*, C.*, D.*, E.* from tblSizes A inner join tblCategories B on B.CatID=A.CategoryID inner join tbBrands C on C.BrandID=A.BrandID inner join tblSubCategories D on D.SubCatID=A.SubCategoryID inner join tblGender E on E.GenderID=A.GenderID", con))
                {
        ...
        private void BindBrand()
        {
            ...
            using (SqlConnection con = new SqlConnection(CS))
            {
                SqlCommand cmd = new SqlCommand("select * from tbBrands", con);
                con.Open();
                SqlDataAdapter sda = new SqlDataAdapter(cmd);
                DataTable dt = new DataTable();
                sda.Fill(dt);
